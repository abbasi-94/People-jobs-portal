  ...
  if (!job) {
    return (
      <div className="text-center py-20">
        ...
        <Link to="/jobs" className="text-blue-600 font-semibold hover:text-blue-700">Browse all jobs</Link>
      </div>
    );
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-xl border border-gray-100 p-6">
              ...
              <div className="prose prose-sm text-gray-600 whitespace-pre-line leading-relaxed">{job.description}</div>
            </div>
            {job.requirements && (
              <div className="bg-white rounded-xl border border-gray-100 p-6">
          ...
          <div className="space-y-4">
              ...
              <div className="space-y-4">
                {job.salary_max > 0 && (
                  <div className="flex items-center gap-3">
                    ...
                    <div><p className="text-xs text-gray-400">Salary</p><p className="text-sm font-semibold text-gray-700">${job.salary_min.toLocaleString()} - ${job.salary_max.toLocaleString()}</p></div>
                  </div>
                )}
                <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="text-xs text-gray-400">Location</p><p className="text-sm font-semibold text-gray-700">{job.location}</p></div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 bg-teal-50 rounded-lg flex items-center justify-center"><Globe className="w-4 h-4 text-teal-600" /></div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-6">
              ...
              <a href={`mailto:${job.employer_email}`} className="flex items-center gap-3 text-sm text-gray-600 hover:text-blue-600 transition-colors">
            ...
            <div className="flex gap-3">
              <a href={`mailto:${job.employer_email}?subject=Application for ${job.title}`} className="flex-1 bg-blue-600 text-white text-center py-3 rounded-xl font-semibold text-sm hover:bg-blue-700 transition-colors">Apply Now</a>
              <button onClick={() => navigator.clipboard.writeText(window.location.href)} className="px-4 py-3 border border-gray-200 rounded-xl text-gray-500 hover:bg-gray-50 transition-colors">
